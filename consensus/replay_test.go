...
func sendTxs(ctx context.Context, cs *State) {
	for i := 0; i < 256; i++ {
		...
		default:
			...
			if _, err := assertMempool(cs.txNotifier).CheckTxSync(tx, mempool.TxInfo{}); err != nil {
				panic(err)
			}
			i++
		}
...
func setupChainWithChangingValidators(t *testing.T, name string, nBlocks int) (*cfg.Config, []*types.Block, []*types.ExtendedCommit, sm.State) {
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(newValidatorTx1, mempool.TxInfo{})
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(updateValidatorTx1, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(newValidatorTx2, mempool.TxInfo{})
	require.NoError(t, err)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(newValidatorTx3, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(removeValidatorTx2, mempool.TxInfo{})
	assert.Nil(t, err)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTxSync(removeValidatorTx3, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
