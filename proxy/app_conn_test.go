...
func TestEcho(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		_, err = proxy.CheckTxSync(context.Background(), &abci.RequestCheckTx{Tx: []byte(fmt.Sprintf("echo-%v", i))})
		if err != nil {
			t.Fatal(err)
		}
	}
...
func BenchmarkEcho(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err = proxy.CheckTxSync(context.Background(), &abci.RequestCheckTx{Tx: []byte("hello")})
		if err != nil {
			b.Error(err)
		}
	}
